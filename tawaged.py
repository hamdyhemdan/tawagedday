    ...
    if st.button("🚀 ابدأ المعالجة الآن"):
        if main_file and source_file:
            try:
                ...
                # تنظيف الأسماء (إزالة المسافات وتحويلها لـ Lowercase للبحث بمرونة)
                df_main.columns = [str(c).strip() for c in df_main.columns]
                df_source.columns = [str(c).strip() for c in df_source.columns]
                
                # --- التعديل الوحيد: إيجاد الأعمدة بمرونة ---
                def find_col(df, target):
                    for col in df.columns:
                        if target.lower() in str(col).lower(): return col
                    return None

                col_id_1, col_date_1 = "رقم البصمه", "التاريخ"
                col_id_2 = find_col(df_source, "Badgenumber")
                col_date_2 = find_col(df_source, "Date")
                target_color_col = "الوقت"
                
                if not col_id_2:
                    st.error(f"خطأ: لم أجد عموداً يحتوي على اسم 'Badgenumber' في ملفك. الأسماء الموجودة هي: {list(df_source.columns)}")
                    return
